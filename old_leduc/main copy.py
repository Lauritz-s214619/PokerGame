...
class Game:
    ...
    def new_round(self):
        ...
        if self.is_pre_flop:
            ...
            #Add blinds to pot
            self.round.pot = self.small_blind + self.big_blind
            self.round.bet_to_call = self.big_blind
        else:
    ...
    def do_action(self, player, action):
        ...
        if action == Actions.Call.value:
            diff = self.round.bet_to_call - player.bet
            ...
            diff = self.round.bet_to_call - player.bet
            raise_amount = diff + self.big_blind
            if raise_amount <= player.wallet:
                player.wallet -= raise_amount
                player.bet += raise_amount
                self.round.bet_to_call = player.bet
                if player.wallet == 0:
                    player.is_all_in = True
                    print("All in!")
        ...
        self.round.num_turns += 1
        #Check if round is done
        bets = [player.bet for player in self.players if player.in_game]
        if bets.count(bets[0]) == len(bets) and self.round.num_turns >= num_active_players:
            self.round.is_done = True
            self.round.num_turns = 0

            self.round.pot = sum(bets)
            for player in self.players:
                player.bet = 0
...
player1.role = Roles.BigBlind
game1 = Game(Deck(), [player1, player2])
game1.deal_cards(2)

print("Round 1:")
game1.new_round()
game1.do_action(Actions.Call.value)
