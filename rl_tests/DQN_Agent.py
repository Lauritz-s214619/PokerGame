...
class DQN(nn.Module):
    def __init__(self):
        ...
        super().__init__()
        self.fc1 = nn.Linear(in_features=72, out_features=144).to(device)
        self.fc2 = nn.Linear(in_features=144, out_features=144).to(device)
        self.out = nn.Linear(in_features=36, out_features=4).to(device)
